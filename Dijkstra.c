...
//structure for saving vertex u and its d(u) value- for heap
struct tuple
{
    int vertex;
    int key;
...
struct tuple extractmin(struct tuple *T[], struct node *A[])
{
    ...
    n--;
    MinHeapify(T, A, 0);
    A[temp->vertex - 1]->delta = temp->key;
    return *temp;
}

//finalise a vertex and relax all its edges
void finalise(struct tuple v, struct node *A[], struct tuple *T[])
{
    ...
    while (temp != NULL)
    {
        m = temp->v;
        i = A[m - 1]->indexinheap;
        //check if the neighbour is already finalised
        if (i == -1)
        {
            temp = temp->next;
            continue;
        ...
        weight = temp->weight;
        //relax edge
        if (T[i]->key > v.key + weight)
        {
            A[m - 1]->pred = v.vertex;
            decreasekey(i, v.key + weight, T, A);
        }
        temp = temp->next;
    }
}

void dijkstras(struct node *A[], struct tuple *T[])
{
    // extract and finalise
    while (n != 0)
    {
