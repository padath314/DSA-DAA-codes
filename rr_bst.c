...
void inorder_nonrec(struct node * curptr)
{
    struct node*  prevptr = curptr;
    while(curptr != NULL)
    {
        if(prevptr == curptr->left)
        {
            printf("%d\n", curptr->key);
...
void RightRotate(struct node ** ptr)
{
    ...
    struct node * lefty = (*ptr) -> left -> right;
    (*ptr) -> left -> right = NULL;
    (*ptr)->left->parent=(*ptr)->parent;
    (*ptr)->left->right=(*ptr);
    (*ptr)=(*ptr)->left;
    (*ptr)->right->left=lefty;
    lefty->parent = (*ptr)->right;
    return;
}
